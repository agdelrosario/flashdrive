<?php
	// This page generates a table of all the records that have expired franchises in the database.
	$query = $_POST;
	...
	if ($qu->num_rows == 0) redirect ('main/');
		...
		foreach ($qu->result() as $row) {
			if ($checker == 0) {
				...
				$checker = 1;
				include("sidebar.php");
				echo "<h1>View list of drivers with expired franchises</h1>";
				...
				if ($query->num_rows == 0) echo "<p>The database is empty.</p>";
					...
					if ($expired == 0) echo "<p>There are no drivers with expired franchises in the database.</p>";
	...
	function displayContent($query, $count, $q, $expired, $role) {
		...
		// Checking if the records in the database have expired franchises.

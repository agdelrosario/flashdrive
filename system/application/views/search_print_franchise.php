<?php
	// PAGE DESCRIPTION: This page displays the search results for expired franchises.
	...
	if ($qu->num_rows == 0) redirect ('main/');
		...
		foreach ($qu->result() as $row) {
			if ($checker == 0) {				
				...
				echo "<h1>Search by franchise Results</h1>";
				...
				if ($query->num_rows == 0) echo "<p>The database is empty.</p>";
					...
					foreach ($query->result() as $row2) {
						...
						$string = strtok ($row2->franchise_exp, "-");
						while ($string != false) {
					...
					if ($expired == 0) echo "<p>The database does not contain any record of a driver with a franchise like what you have searched for.</p>";
